...
public record UpdateLicenciaDTO (
    ...
    @Positive(message="ID de piloto solo puede ser entero positivo") Integer idPiloto,

    //Fecha de vencimiento

    @NotEmpty(message = "Debe contener fecha de vencimiento")
    @Positive(message = "Fecha de vencimiento no puede ser negativa o igual a zero")
    @Digits(integer = 6, fraction=0, message = "La fecha debe contener 6 digitos en formato DDMMAAAA") LocalDate fechaVen,

    //Estado de licencia

    @NotBlank(message="Debe establecer el estado de la licencia: ACTIVA, VENCIDA, SUSPENDIDA ")
